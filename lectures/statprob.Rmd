...
**Population**
:   The entire group of individuals, objects, or data points that we are interested in studying.
...
**Parameter**
:   A numerical characteristic of a **population**. These are typically unknown and what we want to estimate. They are considered fixed values.
...
p2 <- ggplot(data.frame(x=sample_means), aes(x)) + geom_density(fill="blue", alpha=0.4) + labs(title="2. Sampling Distribution of xbar (n=30)", subtitle="Notice it's centered at μ and less spread out")
...
The CLT is the foundation that allows us to build confidence intervals and conduct hypothesis tests for the mean.
